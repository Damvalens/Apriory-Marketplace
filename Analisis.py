...
server = '10.51.80.104,1402'
database = 'pos_central'
username = 'Consulta'
password = '123Consulta123'
...
where  convert(date, vp.fecha) between '2023-01-01' and '2023-03-31'
...
with pd.ExcelWriter('resultado_proveedor.xlsx') as writer:
...
secciones_sub_secc = data.groupby(['Cod_seccion', 'Cod_sub_seccion']).agg(
    {'Total': 'sum', 'Nro_transaccion': 'nunique'}).reset_index()
...
with pd.ExcelWriter('resultado_proveedor.xlsx') as writer:
    ...
    merged_df['division solitos'] = merged_df['Transacciones únicas'] / merged_df['Nro_transaccion']
    # Formatear los valores como porcentaje
    merged_df['division solitos'] = merged_df['division solitos'].apply(lambda x: f"{x:.2%}")
    merged_df['division pareja'] = merged_df['solo_pareja'] / merged_df['Nro_transaccion']
    # Formatear los valores como porcentaje
...
transactions = []
for trans_id, group in data.groupby('Nro_transaccion'):
    transactions.append(list(group['Cod_sub_seccion'].values))
...
# Agrupa las transacciones por sección y calcula el total de transacciones
transactions_per_section = data.groupby('Cod_sub_seccion')['Nro_transaccion'].nunique().reset_index()
transactions_total = data['Nro_transaccion'].nunique()
####################
# Calcular el monto total por Cod_sub_seccion
total_per_section = data.groupby('Cod_sub_seccion')['Total'].sum().reset_index()
...
with pd.ExcelWriter('resultado_subseccion.xlsx') as writer:
    ...
    pd.DataFrame(output_data, columns=['Antecedente', 'Consecuente', 'Número de transacciones']).to_excel(writer,
                                                                                                          sheet_name='Transacciones',
                                                                                                          index=False)
    transactions_per_section.to_excel(writer, sheet_name='Transacciones por sub sección', index=False)
    pd.DataFrame({'Total de transacciones': [transactions_total]}).to_excel(writer, sheet_name='Total', index=False)
# Obtiene las transacciones únicas por sección
unique_transactions = []
for col in data_bin.columns:
...
# Crea un DataFrame con los datos de la salida
output_unique_df = pd.DataFrame(output_data_unique, columns=['Sub Sección', 'Transacciones únicas'])
...
with pd.ExcelWriter('resultado_subseccion.xlsx') as writer:
    ...
    # Crea un DataFrame con los datos de la salida
    output_unique_df = pd.DataFrame(output_data_unique, columns=['Sub sección', 'Transacciones con otras secciones'])
    ...
    with pd.ExcelWriter('resultado_subseccion.xlsx') as writer:
        ...
        # Separa las transacciones únicas y las transacciones con otras secciones
        for trans_id, group in data.groupby('Nro_transaccion'):
            if len(set(group['Cod_sub_seccion'])) == 1:
                unique_transactions.append(trans_id)
            else:
                other_transactions.append(trans_id)
        ...
        print(f'Total de transacciones: {total_transactions}')
        print(f'Transacciones únicas: {unique_transactions_count}')
        print(f'Transacciones con otras sub secciones: {other_transactions_count}')
...
with pd.ExcelWriter('resultado_subseccion.xlsx') as writer:
    ...
    proveedores_subseccion.to_excel(writer, index=False, sheet_name='prov conjuntos ')
    transacciones_por_cliente.to_excel(writer, sheet_name='Transacciones_clientes', index=False)
    clientes_subseccion.to_excel(writer, sheet_name='Total_sub_clientes', index=False)
    clientes_cod_subseccion.to_excel(writer, sheet_name='sub_clientes', index=False)
    monto_promedio.to_excel(writer, sheet_name='Monto_Promedio', index=False)
    # Crear un DataFrame con los resultados
    results_df = pd.DataFrame({'Total de transacciones': [total_transactions],
                               'Transacciones únicas': [unique_transactions_count],
